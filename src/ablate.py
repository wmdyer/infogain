...
def score(nouns, adjs, a_orig, probs, seqs, cl, outfile):
    ...
    outstr = "key\tsurface\ttemplate\tattest\tig_sum\tig_uc_pos\tig_c_pos\tig_uc_neg\tig_c_neg\n"
            ...
            for perm in perms:
                ...
                for w in perm:
                    ...
                    if "/adj" in w and wf in adjs:
                        iga, a, ig_uc, ig_c = partition(adjs, a, probs, wf, 'adj')
                        igs.append(iga)
                    ...
                    if "/noun" in w and wf in nouns:
                        ign, a, ig_uc, ig_c = partition(nouns, a, probs, wf, 'noun')
                        igs.append(ign)
                        ig_parts['c_yes'] += ig_c['yes']
                        ig_parts['uc_yes'] += ig_uc['yes']
                        ig_parts['c_no'] += ig_c['no']
                ...
                if len(igs) == len(words):
                    ...
                    out.append('\t'.join([key, ','.join(perm), template, str(attest), str(np.sum(igs)), str(ig_parts['uc_yes']), str(ig_parts['c_yes']), str(ig_parts['uc_no']), str(ig_parts['c_no'])]) + "\n")
