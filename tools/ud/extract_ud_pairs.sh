if [ "$1" == "" ]
then
    echo "Usage: ./extract_ud_pairs.sh <UD dir> [<more UD dirs>]"
    exit
fi
...
do
	...
	cat $dir/$file | grep "^[0-9][0-9]*"$'\t' | gawk 'BEGIN{FS="\t";C=0;W=""}{if($1=="1") C+=1; if(($2!="_") && (($4=="ADJ" && $8=="amod") || ($4=="NOUN"))) print C":"$1 FS $2"/"$4 FS C":"$7}' >> a
    done
done
