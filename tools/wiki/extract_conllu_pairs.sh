...
join -2 3 -t$'\t' <(cat a | sort -k1,1) <(cat a | sort -k3,3) | gawk 'BEGIN{FS="\t"}{print $5 FS $2}' | grep "ADJ.*NOUN" | tr '[A-Z]' '[a-z]' | tr -d '"' | sort | uniq -c | sed -e 's/^[ ]*//' | tr ' \t' ',' | sed -e 's|/adj||' | sed -e 's|/noun||' | gawk 'BEGIN{FS=","}{if($4=="") print $0}' >> pairs.csv
