...
do
    ...
    printf " pairs1"
    join -t$'\t' -1 2 <(join -t$'\t' <(cat ids | sort -k1,1) <(cat words | sort -k1,1) | sort -k2,2) <(cat words | sort -k1,1) | cut -f3,4 | tr '\t' ',' | grep -E "(NOUN.*ADJ)|(ADJ.*NOUN)" >> syn_pairs.csv
...
echo "validate"
cat syn_pairs.csv | gawk 'BEGIN{FS=","}{if(NF==2) print $0}' | tr -d '"' | sort > temp; mv temp syn_pairs.csv
    
# output counts of triples by type
cat syn_pairs.csv | grep "NOUN.*ADJ" | wc -l | gawk 'BEGIN{FS="\t"}{print "NA: " $1}'
cat syn_pairs.csv | grep "ADJ.*NOUN" | wc -l | gawk 'BEGIN{FS="\t"}{print "AN: " $1}'
